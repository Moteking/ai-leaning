      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-border">
          <div className="px-4 py-4 space-y-3">
            <a href="#features" className="block text-sm text-gray-600 py-2" onClick={() => setIsMenuOpen(false)}>
              機能
            </a>
            <a href="#how-it-works" className="block text-sm text-gray-600 py-2" onClick={() => setIsMenuOpen(false)}>
              使い方
            </a>
            <a href="#pricing" className="block text-sm text-gray-600 py-2" onClick={() => setIsMenuOpen(false)}>
              料金プラン
            </a>
            <a href="#faq" className="block text-sm text-gray-600 py-2" onClick={() => setIsMenuOpen(false)}>
              FAQ
            </a>
            <div className="pt-3 border-t border-border space-y-2">
              <Link href="/auth/login" className="block text-sm text-center text-gray-700 py-2">
                ログイン
              </Link>
              <Link
                href="/auth/register"
                className="block text-sm text-center text-white bg-primary rounded-full py-2.5 font-medium"
              >
                無料で始める
              </Link>
            </div>
