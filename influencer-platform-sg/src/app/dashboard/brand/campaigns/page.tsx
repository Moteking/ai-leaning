...
import { Plus, MoreHorizontal, Calendar, Users, DollarSign, Loader2 } from "lucide-react";
...
interface Campaign {
  ...
  applications: { id: string; creatorId: string; status: string; creator?: { name: string } }[];
...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    ...
    api.campaigns.list(params)
      .then((r) => setCampaigns((r as { data: Campaign[] }).data || []))
              ...
              <div key={campaign.id} className="bg-white rounded-2xl border border-border p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      <span className={`text-xs font-medium px-3 py-1 rounded-full ${statusConfig[campaign.status]?.color || "bg-gray-100"}`}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-border">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <div>
                      <div className="font-medium">{campaign.applications?.length || 0} applicants</div>
                    </div>
                  </div>
                </div>
                ...
                {campaign.applications?.length > 0 && (
                  <div className="pt-4 mt-4 border-t border-border">
                    <div className="text-sm font-medium mb-2">Applicants</div>
                    <div className="flex flex-wrap gap-2">
                      {campaign.applications.map((app) => (
                        <div key={app.id} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border ${
                          ...
                          {app.creator?.name || app.creatorId} ({app.status})
                        </div>
                      ))}
                    </div>
                  </div>
